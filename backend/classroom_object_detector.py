...
import time
from websocket_camera_handler import WebSocketCameraHandler
...
class ClassroomObjectDetector:
    ...
    def run_detection(self, use_websocket=True, camera_id=0):
        ...
        if use_websocket:
            ...
            cap = WebSocketCameraHandler()
            cap.start_websocket_server()

            # Wait for first frame
            print("Waiting for WebSocket frames from browser...")
            while cap.isOpened() and cap.latest_frame is None:
                time.sleep(0.1)
            ...
            # Set camera resolution (only for real camera)
